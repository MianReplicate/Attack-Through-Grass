...
public class GameRendererMixin {
  ...
  @Unique
  private static List<Entity> attackThroughGrass$getAllVehicles(Entity player) {
    ...
    if (original != null && player != null && hitResult.getType() == HitResult.Type.BLOCK) {
      ...
      if (player.level().getBlockState(blockPos).getCollisionShape(player.level(), blockPos).isEmpty()) {
        ...
        if (entityHitResult != null) {
          ...
          // try to pick a solid block between the player and the entity
				  HitResult newResult = pickUncolliding(player, enemyDistance);
  ...
  public static HitResult pickUncolliding(Entity entity, double reach) {
